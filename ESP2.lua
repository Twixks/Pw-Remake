...
local framework = {
    ...
    active_key = Enum.KeyCode.N,
    unload_key = Enum.KeyCode.M,
}
...
do 
    ...
    function framework:calculate_bounding_box(entity)
        if entity.ClassName == "Model" then
            ...
            local height = (Workspace.CurrentCamera.CFrame - Workspace.CurrentCamera.CFrame.Position) * Vector3.new(0, (math.clamp(size.X, 1, 10) + 2) / 2, 0)
...
do 
    function framework:register_player(player)
        ...
        function registry:destruct()
            ...
            registry.update_connection:diconnect() 
